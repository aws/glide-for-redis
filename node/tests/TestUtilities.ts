...
export async function transactionTest(
    ...
    args.push(2);
    baseTransaction.zadd(key15, { one: 1, two: 2 });
    args.push(2);
    baseTransaction.zinter([key13, key15]);
    args.push(["one", "two"]);
    baseTransaction.zinterWithScores([key13, key15]);
    args.push({one: 2, two: 4});
    baseTransaction.zunionWithScores([key13, key15]);
